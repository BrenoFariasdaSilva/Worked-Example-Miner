...
The output of this Python script consists in multiple files according to the number of runs set in the `RUNS` constant. The `OUTPUT_FILE` contains the analysis generated by the Google Generative AI model based on the input data for the respective run. The `MOST_COMMON_OUTPUT_FILE` contains the most common output between the multiple runs. The content saved in the `OUTPUT_FILE` is also showed in the terminal if the `VERBOSE` constant is set to `True`.
