...
5. With the output directory created, we get into the most important step, in which the `traverse_directory` will be executed. This function will open the `CK_METRICS_DIRECTORY_PATH/repository_name` directory and extract the `cbo`, `wmc`, `cboModified` and `rfc` values of each class or method (depending on the type of file you have chosen to process). With that in mind, it will open subfolder inside that directory, in which the name of those subfolder are the metrics generated by ck in every commit hash (this are generated by the execution of the `main.py`), and extract the `cbo`, `wmc`, `cboModified` and `rfc` values of each class or method (depending on the type of file you have chosen to process) file inside the `CK_METRICS_DIRECTORY_PATH/repository_name`. Then, for every commit hash, it will go the class or method name and stores the unique combination of the metrics (cbo, cboModified, wmc and rfc) and the number of times that class or method changed it's metrics and store that data inside the `metrics_track_record` dictionary. So, for every subfolder (commit hash), the `traverse_directory(directory_path)` will call `process_csv_file(file_path, metrics_track_record)` function to get those informations. Finally, the `traverse_directory(directory_path)` will return the `metrics_track_record` dictionary.
