...
11. Finally, the main function will call the `sort_csv_by_percentual_variation(repository_name)` function to sort the dramastic changes csv file named as `{FULL_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}-{DRAMASTIC_CHANGE_METRIC}-{DRAMASTIC_CHANGES_FILENAME}{CSV_FILE_EXTENSION}` by the `Percentual Variation` column, which is the percentual variation of the metrics values.  
