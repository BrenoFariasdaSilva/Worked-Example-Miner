...
4. Now, the `check_ck_metrics_folders` function  will verify if the ck metrics are already calculated. That verification if done by:
   ...
   2. If the csv file exists, it will verify if there is a folder named `repository_name` inside the `CK_METRICS_DIRECTORY_PATH` directory, for example: `/ck_metrics/commons-lang`, in which, it will go through every line in the commit hashes list file and, for every line, it will verify if there is subfolder inside the `CK_METRICS_DIRECTORY_PATH/repository_name` directory, and if inside of it, if there is the files generated by the ck tool, which are defined in the `CK_METRICS_FILES` constant.
