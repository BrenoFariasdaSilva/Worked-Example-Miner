...
This script interfaces with the GitHub API to retrieve and filter repositories according to defined criteria, then randomly selects a subset for further analysis. It is designed to facilitate the selection of repositories for evaluation using the `code_metrics.py` and `metrics_changes.py` scripts. The script produces several outputs: a JSON file with the filtered repositories, a PDF document listing the selected repositories, and a console printout of three randomly chosen repositories from the list.
...
2. `DEFAULT_REPOSITORIES` dictionary in the `code_metrics.py` file, in which you must specify the repository name and the repository url. As this and the `metrics_changes.py` python files, they are responsible for generating the data and metadata of this tool, so, in a first run, select a very small Java repository just to see how it works and get a good idea of the data generated by the tool. As a suggestion, define the constant as follows:
   ...
   ```python
   DEFAULT_REPOSITORIES = {"Zookeeper": "https://github.com/apache/zookeeper"}
   ```
...
11. In this step, we must call the `write_metrics_evolution_to_csv` function. This function will, for each class or method in the repository, write it's values inside the `sorted_metrics_track_record` dictionary to the `{FULL_METRICS_EVOLUTION_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}/{class_name}/{variable_attribute}{CSV_FILE_EXTENSION}` csv file and, at the end of each iteration, it writes the `identifier`, `commit hash`, `occurrences`, and the tree metrics (cbo, wmc and rc) to the metrics evolution file, as well as call the `linear_regression_graphics(metrics, class_name, variable_attribute, repository_name)` function to generate the linear regression graphics for that same class or method. The `linear_regression_graphics` function does the following:
    ...
    3.  Now, it loops through the `metrics` list and, for each metric, it will generate the linear regression graphics and store it in the `{FULL_METRICS_EVOLUTION_DIRECTORY_PATH}/{repository_name}/{CLASSES_OR_METHODS}/{class_name}/{variable_attribute}{CSV_FILE_EXTENSION}` csv file. In this step, many other important things are done, such as:
        ...
        1. Call the `verify_substantial_metric_decrease` function for the specified metric name in the `SUBSTANTIAL_CHANGE_METRIC` constant, which will verify if the current metrics values of the current class or method had a substantial decrease, defined in the `DESIRED_DECREASD` constant. There are restrictions, for example, keywords that might be found in the class name or variable attribute; If it had any decrease equal or higher than the `DESIRED_DECREASE`.
        2. Get the `Refactorings Information` (`Refactorings Types` and `Counter` for each `File Path`), which are generated by calling the `get_refactorings_info` function, which runs the `RefactoringMiner` bin file for an specific commit hash and class name and store the refactorings types and file paths in a dictionary named `refactorings_info`. If the `DESIRED_REFACTORINGS_ONLY` constant is set to `True`, it will only store the substantial changes that are of any of the specified refactorings in the `DESIRED_REFACTORINGS` list. If you want to store all the substantial changes, you must set the `DESIRED_REFACTORINGS_ONLY` constant to `False`.
      ...
      Those information will be saved in a csv file containing the `class_name`, `variable_attribute`, `From metric_value`, `To metric_value`, `Percentual Variation`, `Commit Number`, `Commit Hash`, `Method Invocations` and `Refactoring Patterns` informations. The csv file is stored in the `{FULL_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}/{SUBSTANTIAL_CHANGES_FILENAME}` path.  
...
13. In this step, with the generated statistics stored in the `{METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}-{CHANGED_METHODS_CSV_FILENAME}` CSV file, the main function calls the `sort_csv_by_changes(repository_name)` function. This function performs the following actions: it first verifies if the unsorted CSV file exists at `{FULL_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}/{UNSORTED_CHANGED_METHODS_CSV_FILENAME}`. If the file does not exist, a message is logged, and the function returns without performing any operations. If the file exists, it reads the CSV file into a DataFrame. The function then checks if the DataFrame is empty after the header. If the DataFrame is empty, a message is logged, and the function returns without performing any operations. If the DataFrame contains data, it sorts the DataFrame by the Changed column in descending order, which represents the number of times the metrics changed. The sorted data is then written to a new CSV file located at `{FULL_METRICS_STATISTICS_DIRECTORY_PATH}/{repository_name}/{SORTED_CHANGED_METHODS_CSV_FILENAME}`. This process ensures that the top changes are highlighted and stored in the sorted CSV file for further analysis.
