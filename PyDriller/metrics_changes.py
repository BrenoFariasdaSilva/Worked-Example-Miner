	...
	if FIRST_SUBSTANTIAL_CHANGE_VERIFICATION and verify_file(csv_filename):
		...
		with open(f"{csv_filename}", "w") as csvfile:
			...
			if PROCESS_CLASSES: # If the PROCESS_CLASSES constant is set to True, then we're processing the classes
				writer.writerow(["Class", "Type", f"From {metric_name}", f"To {metric_name}", "Percentual Variation", "Commit Number", "Commit Hash", "Method Invocations", "Detected Refactorings"])
...
def main():
   ...
   """
	...
	print(f"{BackgroundColors.GREEN}This script generates the {BackgroundColors.CYAN}classes or methods metrics evolution history, metrics statistics, linear regression, substantial changes and refactorings{BackgroundColors.GREEN} for the {BackgroundColors.CYAN}{list(DEFAULT_REPOSITORY_NAMES)}{BackgroundColors.GREEN} repositories based on the {BackgroundColors.CYAN}ck metrics files, the commit hashes list file and the diffs of each commit{BackgroundColors.GREEN} generated by the {BackgroundColors.CYAN}./code_metrics.py{BackgroundColors.GREEN} code.{Style.RESET_ALL}", end="\n\n")
