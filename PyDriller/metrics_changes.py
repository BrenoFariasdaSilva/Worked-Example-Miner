...
def process_repository(repository_name, repository_url):
	...
	repositories_attributes = update_repository_attributes(repository_name, time.time() - start_time) if RUN_FUNCTIONS["Update Repositories Attributes"] else None # Update the attributes of the repositories file with the elapsed time and output data size in GB
	write_dict_to_csv(FULL_REPOSITORIES_ATTRIBUTES_FILE_PATH, repositories_attributes) if RUN_FUNCTIONS["Update Repositories Attributes"] else None # Write the updated data back to the CSV file
